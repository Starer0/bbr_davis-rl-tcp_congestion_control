...
class Network():
    ...
    def run_for_dur(self, dur,data):
        ...
        sender_mi = self.senders[0].get_run_data(data)
...
class Sender():
    ...
    def __init__(self, rate, path, dest, features, cwnd=25, history_len=4):
    ...
    def record_run(self,data):
        smi = self.get_run_data(data)
...
class SimulatedNetworkEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        for i in range(0, 1):#len(actions)):
            ...
            action = actions
            delta = action[0]*config.DELTA_SCALE
            #print("Applying delta %f" % delta)
            if delta >= 0.0:
                self.beta = self.beta * (1.0 + delta)
            else:
                self.beta = self.beta / (1.0 - delta)
            if self.beta > 130:
                self.beta = 130
        ...
        IN = c_int*10
        data = IN()
        beta_100 = int(self.beta)
        umsg = str(beta_100)
        testlib1.run(umsg.encode('ascii'),data)

        reward = self.net.run_for_dur(self.run_dur,data)
        for sender in self.senders:
            sender.record_run(data)
        self.steps_taken += 1
    ...
    def reset(self):
        ...
        self.net.run_for_dur(self.run_dur,data)
        self.net.run_for_dur(self.run_dur,data)
        self.reward_ewma *= 0.99
        self.reward_ewma += 0.01 * self.reward_sum
        fo=open("reward.txt","a")
        fo.write("Reward: %0.2f, Ewma Reward: %0.2f\n" % (self.reward_sum, self.reward_ewma))
        fo.close()
        self.reward_sum = 0.0
